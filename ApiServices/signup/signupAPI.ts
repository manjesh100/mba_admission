...
import { TRegisterForm, TVerifyOtp, TOnboardingStepOne } from "@/utils/type";
...
export const signupAPI = createApi({
  ...
  endpoints: (builder) => ({
    ...
    onboardingStepOne:builder.mutation<any, any>({
    query:()=>{
      return {
          url:`${API_ENDPOINT.AUTH.ONBOARDING_STEP_ONE}`,
          method:"POST",
          headers:{
            Authorization:`Bearer ${localStorage.getItem("signupAccesToken")}`,
          }
...
export const {
  useRegisterUserMutation, useVerifyOtpMutation, useResendOtpMutation,useOnboardingStepOneMutation
} = signupAPI;
